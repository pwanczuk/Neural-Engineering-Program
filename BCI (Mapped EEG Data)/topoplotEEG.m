...
function handle = topoplotEEG(Vl,loc_file)

% User Defined Defaults:
MAXCHANS = 256;
DEFAULT_ELOC = 'eloc64.txt';
INTERPLIMITS = 'head';  % head, electrodes
MAPLIMITS = 'maxmin';   % absmax, maxmin, [values]
GRID_SCALE = 67;
CONTOURNUM = 6;
...
fid = fopen(loc_file);
if fid<1,
  error('topoplot(): cannot open eloc_file (%s).\n',loc_file);
  return
end
...
if length(Vl) ~= size(A,1),
 error('topoplot(): data vector must have the same rows (%d) as eloc_file (%d)\n',length(Vl),size(A,1));
end
