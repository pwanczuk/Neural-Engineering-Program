...
for i=1:8
    ...
    for j=1:trial
        spikeTimes=[neuron(find(neuron>go(indDir(j))-1 & neuron<go(indDir(j))+1))-go(indDir(j))]';
        psth(:,i)= psth(:, i) +[histc(spikeTimes,edgesPeri,2)]'/trial/bin;
    end
    z=sum(psth(51:length(edgesPeri),i));
    half=[ half z];
    subplot(3,3,graph(i));
    bar(edgesPeri,psth(:, i));
end
figure;
plot(positions,half,'-*', 'MarkerIndices', 1:45:length(half));
xlabel('Orientation [degrees]');
ylabel('Firing Rate [Hz]');
title("Neuron 1 Tuning Curve");
hold on;
